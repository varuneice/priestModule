...
class ABCalendar extends App {
    ...
    var $monthShortNames = array();
    var $custom_dates = array();
    var $booked_slots = array();
    ...
    function serReservationsInfo_23_may($m, $day, $y) {
    ...
    function serReservationsInfo($m, $day, $y) {
        ...
        $sql = "SELECT * FROM " . $BookingSlotModel->getTable() . " as t1 LEFT JOIN  " . $BookingModel->getTable() . " as t2 ON t1.booking_id = t2.id WHERE (t2.status = 'confirmed' OR (t2.status = 'pending' AND t2.created >= " . $before . " )) AND t1.timestamp BETWEEN " . $from . "  AND " . $to . " AND t1.calendar_id = " . (int)$this->calendar_id . " ";
        $arr = $BookingSlotModel->execute($sql);
        ...
        foreach ($arr as $key => $value) {
            ...
            if (!empty($this->reservationsInfo[$date])) {
                ...
                if($value['location'] == "wholeday" || $value['location'] == "outside" || $value['location'] == "outsidewholeday")
                {
                    $this->reservationsInfo[$date] = $value['location'];
                }

                else
                {
                    $this->reservationsInfo[$date] = (is_numeric($this->reservationsInfo[$date]) ? (int)$this->reservationsInfo[$date] : 0) + (int)$value['count'];
                }
    ...
    function getMonthView($month = null, $year = null) {
        ...
        $arr1 = $RentalBookingSlotModel->getAllRentalBookingSlotData();
        $cid = $_GET['cid'];
        if (!empty($month)) {
            $this->month = $month;
        }
...
// 21-APR-2025
                $tooltip = '';
                if($cid == "2"){
                    $CheckDate = str_pad($day, 2, '0', STR_PAD_LEFT) . "-" . str_pad($this->month, 2, '0', STR_PAD_LEFT) . "-" . $this->year;
                    $selectedLocation  = $this->GetNewSelectedLocations($CheckDate);
                    
                    if(!empty($selectedLocation)){
                        if (strtolower($selectedLocation) === 'both') {
                            $tooltip = "Fully Booked";
                        } elseif (strpos($selectedLocation, ',') !== false) {
                            $tooltip = "Fully Booked";
                        } else {
                            $tooltip = $selectedLocation . " Not Available";
                        }
                    }
                ...
                if (!empty($this->custom_dates[$timestamp])) {
                    ...
                    $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $timestamp), (int)date('j', $timestamp), (int)date('Y', $timestamp));
                    $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $timestamp), (int)date('j', $timestamp), (int)date('Y', $timestamp));
        ...
        if ($date >= $now) {
            ...
            if (!empty($this->slots)) {
                if (!empty($this->custom_dates[$date])) {
                    ...
                    $start_time = explode(':', $this->custom_dates[$date]['start'] ?? '00:00');
                    $end_time = explode(':', $this->custom_dates[$date]['end'] ?? '00:00');
                    $launch_start_time = explode(':', $this->custom_dates[$date]['lunch_start'] ?? '00:00');
                    $launch_end_time = explode(':', $this->custom_dates[$date]['lunch_end'] ?? '00:00');
                    $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                    $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                    ...
                    switch (date('N', $date)) {
                        case 1:
                            $count = $this->slots['monday_count'];
                            $start_time = explode(':', $this->slots['monday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['monday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['monday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['monday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['monday_slot_lenght'];
                            $is_day_off = $this->slots['monday_is_day_off'];
                        ...
                        case 2:
                            $count = $this->slots['tuesday_count'];
                            $start_time = explode(':', $this->slots['tuesday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['tuesday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['tuesday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['tuesday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['tuesday_slot_lenght'];
                            $is_day_off = $this->slots['tuesday_is_day_off'];
                        ...
                        case 3:
                            $count = $this->slots['wednesday_count'];
                            $start_time = explode(':', $this->slots['wednesday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['wednesday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['wednesday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['wednesday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['wednesday_slot_lenght'];
                            $is_day_off = $this->slots['wednesday_is_day_off'];
                        ...
                        case 4:
                            $count = $this->slots['thursday_count'];
                            $start_time = explode(':', $this->slots['thursday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['thursday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['thursday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['thursday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['thursday_slot_lenght'];
                            $is_day_off = $this->slots['thursday_is_day_off'];
                        ...
                        case 5:
                            $count = $this->slots['friday_count'];
                            $start_time = explode(':', $this->slots['friday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['friday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['friday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['friday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['friday_slot_lenght'];
                            $is_day_off = $this->slots['friday_is_day_off'];
                        ...
                        case 6:
                            $count = $this->slots['saturday_count'];
                            $start_time = explode(':', $this->slots['saturday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['saturday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['saturday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['saturday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['saturday_slot_lenght'];
                            $is_day_off = $this->slots['saturday_is_day_off'];
                        ...
                        case 7:
                            $count = $this->slots['sunday_count'];
                            $start_time = explode(':', $this->slots['sunday_start'] ?? '00:00');
                            $end_time = explode(':', $this->slots['sunday_end'] ?? '00:00');
                            $launch_start_time = explode(':', $this->slots['sunday_lunch_start'] ?? '00:00');
                            $launch_end_time = explode(':', $this->slots['sunday_lunch_end'] ?? '00:00');
                            $launch_start = mktime((int)($launch_start_time[0] ?? 0), (int)($launch_start_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $launch_end = mktime((int)($launch_end_time[0] ?? 0), (int)($launch_end_time[1] ?? 0), 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                            $slot_lenght = $this->slots['sunday_slot_lenght'];
                            $is_day_off = $this->slots['sunday_is_day_off'];
            ...
            if (!empty($start_time) && !empty($end_time) && !empty($count)) {
                $start_time = mktime((int)$start_time[0], (int)$start_time[1], 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
                $end_time = mktime((int)$end_time[0], (int)$end_time[1], 0, (int)date('n', $date), (int)date('j', $date), (int)date('Y', $date));
            ...
            if (!empty($is_day_off) && $is_day_off == 1) {
             ...
             elseif( (!empty($this->reservationsInfo[$date]) && ($this->reservationsInfo[$date] == "wholeday" || $this->reservationsInfo[$date] == "outside" || $this->reservationsInfo[$date] == "outsidewholeday" )))
            {
                $class = 'gzABCalCellReserved';
            }
    ...
    // 21-APR-2025 for get booked locations
    function GetNewSelectedLocations($bookingDate){
        $RentalBookingSlotModel = new RentalBookingSlotModel();
        $opts = array();
        $date = isset($bookingDate) ? trim($bookingDate) : null;
        $location = $RentalBookingSlotModel->getBookingRentalLocation($date);
        return $location;
    }
