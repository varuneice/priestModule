...
<input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
        ...
        <input type="hidden" name="stripeToken" id="stripeToken" value="" />
         <input type="hidden" name="event" id="event" value="event2" />
  ...
  $(this).val($(this).val().replace(/[^a-z0-9]/gi, ''));
    ...
    function eventcurrentrun() {
        ...
        $.ajax({
            ...
            url: "<?= INSTALL_URL ?>load.php?controller=Event&action=checkdatevalid2&cid",  
            success: function (res) {
...
$(function () {
    ...
    console.log('EVENT2 ZELLE DEBUG SCRIPT LOADED');

    $('#PaymentOption').on('change', function () {
        var val = $(this).val();
        debugger;
        console.log('EVENT2 PAYMENT OPTION CHANGE', {
            paymentOption: val,
    ...
    $('#zelle-modal-paid-btn').on('click', function () {
        debugger;
        console.log('EVENT2 ZELLE COMPLETED BUTTON CLICKED');
    ...
    $('#checkPaymentData').on('click', function () {
        ...
        debugger;
        console.log('EVENT2 MANUAL ZELLE SEARCH', {
            donorName: donorName,
            zelleAmt: zelleAmt,
            zelleDate: zelleDate,
            account: $.trim($('#account_type').text())
        ...
        $.ajax({
            ...
            success: function (res) {
                debugger;
                console.log('EVENT2 MANUAL ZELLE RESPONSE', res);
    ...
    $('#MemberID').on('change', function () {
        ...
        debugger;
        console.log('EVENT2 SELECTED ZELLE TRANSACTION', {
            selectedValue: dd,
    ...
    function doZelleImportSearch() {
        ...
        debugger;
        console.log('EVENT2 AUTO ZELLE SEARCH', {
            regVal: regVal,
            donorName: donorName,
            zelleAmt: zelleAmt,
            today: today,
        ...
        $.ajax({
            ...
            success: function (res) {
                debugger;
                console.log('EVENT2 AUTO ZELLE RESPONSE', res);
                var trimmed = $.trim(res);
