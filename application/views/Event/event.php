...
<input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
        ...
        <tr class="tr">
            ...
            <td class="td"><input required="" id="Email" class="form-control input-sm" type="text"
                    ...
                    value="<?php echo $tpl['arr']['email'] ?? ''; ?>" pattern="[^@\s]+@[^@\s]+\.[^@\s]+"
                    ...
                    if (descrip.length) {
                        ...
                        if(evimg == "")
                        {
                            ...
                            if (test !== null || test !== undefined) {
                                ...
                                //  addRow += "<table>";
                                for (let i = 0; i < data - 1; i++) {
                                    addRow += "<label style='color:gray; font-size:1.5rem; margin-top:2%;'>" + test[i] + "</label></br>"
...
$(function () {
    ...
    // Manual verify — Verify Zelle Payment button
    $('#checkPaymentData').on('click', function () {
        ...
        debugger;
        console.log('MANUAL ZELLE SEARCH', {
            donorName: donorName,
            zelleAmt: zelleAmt,
            zelleDate: zelleDate,
        ...
        $.ajax({
            ...
            success: function (res) {
                debugger;
                console.log('MANUAL ZELLE RESPONSE', res);
    ...
    // MemberID dropdown change — validate amount match
    $('#MemberID').on('change', function () {
        ...
        if (tot === price) {
            ...
            $("#payment_btn_id").addClass('disabled');
            alert('Selected Zelle amount does not match the event amount. Please select the correct transaction.');
        }
    });
    ...
    function doZelleImportSearch() {
        ...
        $.ajax({
            ...
            success: function (res) {
                debugger;
                console.log('AUTO ZELLE RESPONSE', res);
                var trimmed = $.trim(res);
