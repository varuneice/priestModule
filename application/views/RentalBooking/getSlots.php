<div class="GZBookingContainer">
    <?php
    $getTimePart = function ($parts, $index) {
        return isset($parts[$index]) && is_numeric($parts[$index]) ? (int)$parts[$index] : 0;
    };
    ?>
            ...
            <table class="table table-bordered table-hover table-striped">
                ...
                <tbody>
                    ...
                    if (!empty($tpl['custom_dates'])) {
                        ...
                        $launch_start = mktime($getTimePart($launch_start_time, 0), $getTimePart($launch_start_time, 1), 0, date('n', $date), date('j', $date), date('Y', $date));
                        $launch_end = mktime($getTimePart($launch_end_time, 0), $getTimePart($launch_end_time, 1), 0, date('n', $date), date('j', $date), date('Y', $date));
                    ...
                    $slot_lenght = (int)($slot_lenght ?? 0);
                    $count = (int)($count ?? 0);
                    if (empty($start_time) || empty($end_time) || $slot_lenght <= 0) {
                        return;
                    }
                    for ($i = mktime($getTimePart($start_time, 0), $getTimePart($start_time, 1), 0, date('n', $date), date('j', $date), date('Y', $date)); $i < mktime($getTimePart($end_time, 0), $getTimePart($end_time, 1), 0, date('n', $date), date('j', $date), date('Y', $date)); $i += $slot_lenght * 60) {
