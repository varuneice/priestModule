...
abstract class BaseQuery implements IteratorAggregate {
    ...
    protected function __construct(Model $fpdo, $clauses) {
        $this->fpdo = $fpdo;
    ...
    public function execute() {
        ...
        if ($this->fpdo->getPdo()) {
            //if (method_exists($this->fpdo->getPdo(), 'prepare')) {
        ...
        // PHP 8.0+ PDO default error mode is ERRMODE_EXCEPTION; wrap to preserve
        // the PHP 7 behaviour of returning false on error rather than throwing.
        $time = microtime(true);
        try {
            ...
            error_log('[BaseQuery] PDOException: ' . $e->getMessage() . ' | Query: ' . $this->buildQuery());
            @file_put_contents(sys_get_temp_dir() . '/hdbs_debug.log', date('Y-m-d H:i:s') . ' [BaseQuery] PDOException: ' . $e->getMessage() . ' | Query: ' . $this->buildQuery() . PHP_EOL, FILE_APPEND);
            $result = false;
        }
    ...
    private function clauseNotEmpty($clause) {
        if ($this->clauses[$clause]) {
            // PHP 8 removed count() on non-arrays (TypeError). Strings (e.g. the UPDATE
            // table name) must be treated as non-empty, not counted as zero.
